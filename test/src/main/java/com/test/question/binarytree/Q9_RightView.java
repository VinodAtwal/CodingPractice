...
public class Q9_RightView {
    public static void righttView(Node node , Integer height,List<Integer> rightView  ){
        if(rightView.size()==height){
            rightView.add(node.data);
        }
        if(node.right!= null){
            righttView(node.right,height+1,rightView);
        }
        if(node.left!= null) {
            righttView(node.left, height + 1,rightView);
        }

    }


    public static void rightViewIterative(Node node){
